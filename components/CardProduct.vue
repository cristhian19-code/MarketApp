<template>
  <div>
    <vs-card type="1">
      ...
      <template #text>
        <h3>S/. {{ price }} {{ measurement }}</h3>
      </template>
      <template #interactions>
        <vs-button
          success
          icon
          @click="AddProduct({ name, price, url, cant, measurement })"
        >
          <i class="bx bx-plus"></i>
        </vs-button>
        <vs-input type="number" v-model.number="cant" max="50" min="0" />
        <vs-button
          class="btn-chat"
          shadow
          primary
          @click="activeComment = !activeComment"
        >
