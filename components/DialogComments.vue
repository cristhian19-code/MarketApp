    ...
    <div class="comments">
      <vs-dialog full-screen overflow-hidden auto-width v-model="activeComment">
        ...
        <div class="container-comments">
          <CommentVue v-for="(comment,index) in comments" :key="index" :avatar="comment.avatar" :name="comment.name" :description="comment.comment"/>
        </div>
        <template #footer>
            <vs-input
              type="text"
...
export default {
  ...
  methods:{
    async Comment(){
        ...
        if(!res.data.error){
          this.openNotification('top-right','success','Comentario', res.data.message)
        }else{
          this.openNotification('top-right','error','Comentario', res.data.message)
        }
    },
